...
List dbDocument = [];
class MuseumMusicBody extends StatefulWidget{

  @override
  State<StatefulWidget> createState() {
    Global.db.collection('achievement').where({'label': '音乐'}).get().then((res) {
      dbDocument = res.data;
      print(dbDocument);
    });
    return MuseumMusicBodyState();
  }

}

class MuseumMusicBodyState extends State<MuseumMusicBody> {
  getDocument() async{
    Global.db.collection('achievement').where({'label': '音乐'}).get().then((res) {
      setState(() {
        dbDocument = res.data;
