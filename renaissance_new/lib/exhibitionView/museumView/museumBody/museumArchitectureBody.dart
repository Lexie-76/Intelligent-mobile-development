...
List dbDocument = [];
class MuseumArchitectureBody extends StatefulWidget{

  @override
  State<StatefulWidget> createState() {
    Global.db.collection('achievement').where({'label': '建筑'}).get().then((res) {
      dbDocument = res.data;
      print(dbDocument);
    });
    return MuseumArchitectureBodyState();
  }

}

class MuseumArchitectureBodyState extends State<MuseumArchitectureBody> {
  getDocument() async{
    Global.db.collection('achievement').where({'label': '建筑'}).get().then((res) {
      setState(() {
        dbDocument = res.data;
